...
import { editdoctorprocess } from '../../admin/APIs/doctorAPI'
  ...
  const handlesubmit = (e) => {
    ...
    const { name, pictureUrl, email, address, phoneNumber, clinicAddress, clinicName, workHours, specialization } = formdata;
      ...
      editdoctorprocess(data,doctorId).then((response) => {
        ...
        //history.push(`/doctorpage/profile/${doctorId}`)
        setformdata({
          name:name,
    imagefile:null,
  ...
  const showtheform = () => {
    return(
        ...
        {succesmsg && successmessage(succesmsg)}
            <h1 style={{textAlign:'center',paddingTop:'30px'}}>Edit Profile</h1>
